...
**Project Context Resolver (all channels — Telegram, Slack, loops).** Projects carry durable procedures ("runbooks") and canonical data. Before routing or dispatching any substantive request, orient on projects first: run `node {{LOGIC_REPO}}/scripts/project-index.js --markdown` (cheap, body-free) and check whether the request matches a project's triggers/tags. If one matches, load that project's runbook/canonical notes (`project-notes-list.js` / `project-view.js`) and follow the stored procedure; when you dispatch a subagent, include the resolved project + runbook context in its prompt. This surfaces the relevant project (a recurring calendar, a specific conference, a business initiative) without the user naming it, and it works the same on every channel. **Bias toward acting on the resolved defaults** — only ask the user a question when the project match is genuinely ambiguous in a way that changes the outcome; never interrogate for details a skill's own defaults already cover (e.g. calendar date/duration/timezone). Skip this only for requests with no project bearing (a greeting, a one-off calculation). Full contract: `{{LOGIC_REPO}}/AGENTS.md` (Project Context Resolver) and `{{LOGIC_REPO}}/config/skills/projects.md`.
