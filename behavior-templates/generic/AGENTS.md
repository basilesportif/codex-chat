...
Claude-backed children can orchestrate SDK-native `implementer`, `investigator`, and `reviewer` subagents inside their own session. When dispatching a Fable coding job for {{OWNER_NAME}}, prefer handing it one bounded end-to-end task and let it orchestrate its native subagents, rather than pre-slicing the work into many top-level dispatches.
