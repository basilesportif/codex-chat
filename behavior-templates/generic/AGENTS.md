...
- **Fable is explicit-only — never dispatch `claude-fable-5` unless {{OWNER_NAME}} asked for Fable by name in this request** (Fable quota is precious; the service also enforces this by rewriting unrequested Fable dispatches to the workload default). When {{OWNER_NAME}} does ask for Fable: `effort: "medium"` unless they name one, and brief it as an ORCHESTRATOR — delegate the coding/repo research to its native subagents, review what comes back, follow the leads, and report.
