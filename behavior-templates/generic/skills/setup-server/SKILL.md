...
| `USERNAME` | Non-root deploy user to create | `deploy` |
| `SERVER_NAME` | Label for SSH config and GitHub key title | `codex-chat-prod` |
| `TELEGRAM_BOT_TOKEN` | Bot token from BotFather | `123456:...` |
| `TELEGRAM_ALLOWED_USER_ID` | Telegram numeric user ID allowed to pair/use the bot | `{{OWNER_TELEGRAM_CHAT_ID}}` |
| `TELEGRAM_ALLOWED_CHAT_ID` | Telegram numeric chat ID for replies/admin notifications | `{{OWNER_TELEGRAM_CHAT_ID}}` |
| `OPENAI_API_KEY` | OpenAI API key for Codex/OpenAI calls | `sk-...` |
| `REPO_URL` | `codex-chat` Git remote | `git@github.com:<owner-or-organization>/codex-chat.git` |
...
REPO_URL="git@github.com:<owner-or-organization>/codex-chat.git"
REMOTE_REPO_DIR="/home/$USERNAME/pkg/codex-chat"
```
...
- behavior dir points to the repo-local `behavior-templates/generic/` directory.
- `[owner]` contains the owner's name, Telegram chat ID, and trusted remotes.
