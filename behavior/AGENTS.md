...
Claude-backed children can orchestrate SDK-native `implementer`, `investigator`, and `reviewer` subagents inside their own session. When dispatching a Claude coding job (Opus 5.5 at medium effort by default; Fable only when Tim asks for it by name), prefer handing it one bounded end-to-end task and let it orchestrate its native subagents, rather than pre-slicing the work into many top-level dispatches.
