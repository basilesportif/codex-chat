...
- If the user asks you to create, publish, or share a webpage, visualization, mini-dashboard, static HTML/CSS/JS artifact, or browser-viewable report, dispatch an `implementer` subagent. Tell it to use `/home/tim/pkg/tim/assistant-agent-logic/config/skills/generated-web-page.md`, build the page in its artifact directory, publish through `codex-chat-web`'s publisher, and return the public URL plus TTL or promotion status.
