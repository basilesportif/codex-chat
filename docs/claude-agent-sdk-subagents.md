...
| Complex agentic coding / enterprise work | `claude-opus-5-5` | Current flagship Opus and the Claude-mode coding default (medium effort; high for very intensive work). Supports fast mode. `opus` resolves to Opus 5.5 on Claude Code 2.1.280+ (Agent SDK 0.3.280+; older bundled CLIs map `opus` to Opus 5). `claude-opus-5` and `claude-opus-4-8` remain available as pinned older snapshots. |
...
- `model` is passed through to the Claude Agent SDK unchanged. For Opus 5.5, dispatch with `model: "claude-opus-5-5"`; for Fable 5.1, dispatch with `model: "claude-fable-5-1"`.
...
- `implementer` writes and edits code and runs relevant tests/builds. Its model comes from `[subagents.claude].implementerModel` (default `claude-opus-5-5`) and its effort from `implementerEffort` (default `medium`).
- `investigator` performs read-only code, repository, and log research. Its model comes from `[subagents.claude].investigatorModel` (default `sonnet`) and its effort from `investigatorEffort` (default `medium`); edit tools are explicitly disallowed.
- `reviewer` performs findings-first, read-only code review. Its model comes from `[subagents.claude].reviewerModel` (default `claude-opus-5-5`) and its effort from `reviewerEffort` (default `medium`).

The corresponding environment overrides are `CODEX_CHAT_SUBAGENTS_CLAUDE_IMPLEMENTER_MODEL`, `CODEX_CHAT_SUBAGENTS_CLAUDE_INVESTIGATOR_MODEL`, and `CODEX_CHAT_SUBAGENTS_CLAUDE_REVIEWER_MODEL` (plus `..._IMPLEMENTER_EFFORT`, `..._INVESTIGATOR_EFFORT`, `..._REVIEWER_EFFORT`). Aliases and full Claude model IDs are passed through to the SDK unchanged.
...
The nested-agent predicate, the drained-nudge text, the preamble rules, and the `PreToolUse` hook builder live in `src/claude-nested-agents.ts`, shared with the Claude main loop, which applies the same three guards to its user-facing turns (`docs/claude-main-loop.md`).
...
2. Execute through an internal wrapper that reuses `sanitizeCodexChildProcessEnv`, fixed `codex exec --json --output-last-message ... --model gpt-6-astra -c model_reasoning_effort=high --sandbox <configured> -c ask_for_approval=<configured>`, with workspace pinned to `config.service.workspace` and artifacts written under the parent Claude job directory.
...
5. Add cost/rate-limit controls before enabling by default: explicit config flag, model allowlist, concurrency limit, timeout, and a clear error surface when the Codex provider rejects `gpt-6-astra` or the requested service tier.
