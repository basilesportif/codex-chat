...
The end goal is that any scratch pages Tim creates are private only to the allowed users authenticated to `me.galebach.com` using a simple Clerk setup. Move generated Codex Web scratch pages from public-by-obscurity hosting to server-side Clerk-protected private pages using the existing Caddy deployment as a simple reverse proxy to an ExpressJS serving program. Preserve current public `/pages/*` behavior during migration.
...
1. **Investigate Tim's continual learning repo as the Clerk reference**
   - Before coding product changes, inspect the authoritative `tim-continual-learning` repo from the repo registry: `tim@89.167.72.52:~/pkg/mush/tim-continual-learning`.
   - Use that repo as the model/reference for Tim's preferred simple Clerk setup, sign-in flow, environment naming, and deployment conventions.
...
2. **Add Clerk-protected ExpressJS serving program**
   ...
   - Unauthenticated curl requests to protected HTML or assets must receive `401`/blocked, not page content.
...
6. **Migrate publisher defaults to private**
   ...
   - Keep explicit `--runtime-root /srv/codex-chat-web/pages --public-base-url https://me.galebach.com/pages` available for intentional legacy/public publishes.
...
```sh
curl -si https://me.galebach.com/private/pages/$ID/ | head
curl -si https://me.galebach.com/private/pages/$ID/app.js | head
curl -I  https://me.galebach.com/pages/$ID/
```

Expect private routes to be `401`/blocked and legacy public path to be `404` for a new private ID.
...
- Click username; confirm logout; reload private URL blocked/sign-in.
...
3. What is the exact Clerk Account Portal/sign-in URL for the single Clerk application?
