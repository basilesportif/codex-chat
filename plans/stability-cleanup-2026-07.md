...
- [x] **9. Scripts bypassing transaction lock** (B commit b8e8cac; error paths release locks before exit; B suite 66/66) — B `scripts/`: `urgent-email.js`, `urgent-message.js`, `dismiss-email.js`, `dismiss-message.js`, `flag-event.js`, `calendar-allowlist.js`, `calendar-check-invites.js`, `protonmail-send.js` do load()…await network…save() as separate lock acquisitions. Convert each RMW to `store.transaction()`; do network fetches outside, apply inside.
