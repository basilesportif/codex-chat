...
describe("admin routes", () => {
  ...
  test("Slack config route accepts null optional fields when previous Slack config is absent", async () => {
    const root = await tempRoot();
    const envFile = join(root, "env");
    const { baseUrl } = await apiHarness(envFile, {
      adminAuthDeps: {
    ...
    const authed = await authFetch(
      ...
      "/api/admin/codex-chat/slack-config",
      {
        method: "POST",
        body: JSON.stringify({
          signingSecret: "optional-fields-signing-secret",
          botToken: null,
          appToken: null,
          baseUrl: null,
          eventsPath: null,
    ...
    expect(JSON.stringify(payload)).not.toContain(
      "optional-fields-signing-secret",
    );
    const written = await readFile(envFile, "utf8");
    expect(written).toContain(
      "SLACK_SIGNING_SECRET='optional-fields-signing-secret'",
    );
    expect(written).not.toContain("SLACK_BOT_TOKEN=");
    expect(written).not.toContain("SLACK_APP_TOKEN=");
