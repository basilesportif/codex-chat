...
async function writeTestBrainCapabilityStore(path: string): Promise<void> {
  const operations = [
    "telegram.event.receive", "assistant.run", "service.command.logs", "service.command.help", "runtime.status.read",
    "subagents.dispatch", "subagents.control.cancel", "subagents.control.steer", "subagents.backend.set",
    ...
    "system.callback.enqueue", "employees.manage.list", "employees.manage.status", "employees.manage.start", "employees.manage.stop", "employees.manage.steer"
  ];
  const selectors = { source: "*", surfaceKind: "*", chatId: "*", messageId: "*", conversationSessionId: "*", actorId: "*", targetId: "*", targetPolicy: "*", outputType: "*" };
  await writeFile(path, JSON.stringify({
    schemaVersion: 2,
    people: [{ id: "person_tim", status: "active", primarySubjectId: "person:person_tim", subjectIds: ["person:person_tim"] }],
    externalIdentities: [{ id: "identity_telegram_tim", provider: "telegram", providerUserId: "253768951", personId: "person_tim", status: "linked" }],
    subjects: [{ id: "person:person_tim", personId: "person_tim" }, { id: "system:system" }],
    grantBundles: [],
    grants: ["person:person_tim", "system:system"].flatMap((subjectId) => operations.map((operation) => ({
      id: `grant_${subjectId.replace(/[^a-z0-9]+/gi, "_")}_${operation.replace(/[^a-z0-9]+/gi, "_")}`,
      subjectId, capabilityId: operation, grantKind: "capability", resource: { kind: "global", id: "*", selectors },
      actions: ["*"], status: "active", enforcement: "enforcing", grantedAt: "2026-07-04T00:00:00.000Z"
    })))
  }, null, 2));
}
